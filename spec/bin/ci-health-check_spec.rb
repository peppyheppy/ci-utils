  ...
  describe "DO IT LIVE on staging" do
    ...
    ['s', ''].each do |s| 
      before :each do
        ...
        # can't stub a command line tool's requests: stub_http_request(:get, /.*wildfire.*/).to_return(:status => [401, "Not Authorized"])
        @live_staging_url = "http#{s}://account.wildfirestaging.com/health"
      end
      ...
      it "should display be authorized if valid basic auth is sent over" do
        ...
        run_local_command "ci-health-check #{@live_staging_url} --auth wildfire:A1wNXX4YufOpPI" do |result,stdout,stderr|
          result.should == 0
          stdout.should =~ /OK/
        end
      end
